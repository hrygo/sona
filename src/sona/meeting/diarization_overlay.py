"""会议分人 overlay：用非流式 diarize 修正流式转录的说话人归属。

背景：流式 WS 路径（SpeechRail 缺陷未修前）无法下发带 speaker 的 segment 事件，
会议转录的说话人恒为 ``speaker:0``。本模块在流式之外，用
:class:`~sona.speechrail.batch_transcriber.SpeechRailBatchTranscriber` 把会议期间
缓冲的 PCM 提交到非流式 diarize 端点，拿到**词级/短句级带 speaker 的结果**，
再按**时间戳重叠**把说话人标签归属到已确认的流式转录段上。

关键取舍：采用**标签归属（label-mapping）而非文本替换**。流式段已含用户实时看到
的文本，且其时间戳来自流式窗口；非流式 diarize 的文本与流式可能存在轻微 ASR 差异。
按时间重叠仅修正 speaker_key，保留流式段文本，避免两路 ASR 不一致造成文本跳变。

S0 约束：overlay 已默认关闭，只保留为**显式选择的 legacy 诊断路径**（新分人扩展
启用时必须完全停用，见 :func:`overlay_gate_for_runtime`）。缓冲以分块 deque 维护
样本起点（``buffer_start_sample``）：超限丢弃最旧分块并推进起点，flush 输出的
span 与覆盖窗口都换算到**会议时间线**；只允许修改被缓冲音频完整覆盖的 segment，
跨裁剪边界段不改。请求长度限制在调用前置校验，失败可见，不静默吞掉。
"""

from __future__ import annotations

import hashlib
import logging
from collections import deque
from collections.abc import Sequence
from dataclasses import dataclass
from typing import Protocol

from sona.meeting.models import NormalizedSegment
from sona.speechrail.batch_transcriber import MAX_DIARIZE_PCM_BYTES

logger = logging.getLogger(__name__)

# 16 kHz mono s16le = 32_000 bytes/sec
_BYTES_PER_SEC = 32_000
_SAMPLES_PER_SEC = 16_000
_SAMPLES_PER_MS = _SAMPLES_PER_SEC // 1000

# 最小可提交分人的音频时长（避免空/静音片段浪费一次推理，约 0.8s）
_MIN_FLUSH_BYTES = int(0.8 * _BYTES_PER_SEC)


@dataclass(frozen=True, slots=True)
class SpeakerLabelledSpan:
    """一段已归属说话人的会议时间线区间（用于往后端段映射）。"""

    speaker_key: str
    # 原始匿名标签（如 ``spk_01``）；仅诊断用途。
    raw_speaker: str
    start_ms: int
    end_ms: int


@dataclass(frozen=True, slots=True)
class OverlayFlushResult:
    """一次 overlay flush 的产出：会议时间线 span 与被缓冲音频覆盖的窗口。

    ``covered_*_ms`` 为 ``None`` 表示本次没有可归属的缓冲窗口；调用方只能修改
    完整落在覆盖窗口内的 segment。
    """

    spans: tuple[SpeakerLabelledSpan, ...] = ()
    covered_start_ms: int | None = None
    covered_end_ms: int | None = None


class BatchTranscriber(Protocol):
    """非流式 diarize 的窄端口；便于测试注入假实现。"""

    async def transcribe_diarize(self, pcm: bytes) -> object: ...


def meeting_range(buffer_start: int, start: int, end: int) -> tuple[int, int]:
    """把缓冲相对样本区间 ``[start, end)`` 换算为会议时间线样本区间。

    ``buffer_start`` 是当前缓冲首字节对应的会议样本起点（裁剪丢弃旧音频后
    不再是 0）。输出可直接用于样本/毫秒换算，保证裁剪后 span 不再误配会议
    开头的历史段。
    """
    if end < start:
        raise ValueError("end 不得小于 start")
    return (buffer_start + start, buffer_start + end)


def overlay_gate_for_runtime(*, overlay_enabled: bool, extensions_enabled: bool) -> bool:
    """诊断 overlay 的运行时启用门。

    只有显式开启 legacy 诊断**且**未启用新分人扩展时才允许会末 batch 调用；
    扩展启用后即使旧开关残留 true 也必须停用。
    """
    return overlay_enabled and not extensions_enabled


def meeting_diarization_group_id(owner: str | None) -> str:
    """把应用层会议 owner 映射到协议层匿名 group id。

    与流式路径（``subtitles/sessions.py::_diarization_group_id``）保持完全一致：
    ``sha256(owner)``，保证 overlay 修正的 speaker_key 与流式 confirmed 段落在
    **同一个命名空间**，否则 rename/remap 的 ``speaker_key`` 精确匹配会失效。
    """
    if not owner:
        raise ValueError("meeting diarization requires a capture owner")
    return hashlib.sha256(owner.encode("utf-8")).hexdigest()


def meeting_speaker_key(group_id: str, raw_speaker: str) -> str:
    """把 SpeechRail 匿名标签映射到会议命名空间。

    与会话流式路径（``transcriber._speaker_key``）保持一致：``spk_01`` →
    ``group:{group_id}:speaker:spk_01``。``group_id`` 必须传
    :func:`meeting_diarization_group_id` 推导出的协议层 group id。
    """
    label = raw_speaker.strip()
    if not label:
        label = "0"
    return f"group:{group_id}:speaker:{label}"


def assign_speakers_by_overlap(
    segments: Sequence[NormalizedSegment],
    spans: Sequence[SpeakerLabelledSpan],
) -> dict[str, str]:
    """按时间重叠把说话人 span 归属到 confirmed 转录段。

    对每个 confirmed 段，在与之重叠的 span 中取**重叠时长最大**的那个说话人；
    无重叠时保守地归到无说话人（``speaker:0`` 不变）。返回
    ``{segment.id: speaker_key}`` 映射，只包含确实改动的段。调用方负责保证
    ``segments``/``spans`` 都已限定在被缓冲音频覆盖的窗口内。
    """
    if not spans:
        return {}
    # 归一化：跨批量 span 时保证同一说话人 label 稳定（span 已映射，这里直接按 key）。
    result: dict[str, str] = {}
    for segment in segments:
        candidates: dict[str, int] = {}
        for span in spans:
            overlap = min(segment.end_ms, span.end_ms) - max(segment.start_ms, span.start_ms)
            if overlap > 0:
                candidates[span.speaker_key] = candidates.get(span.speaker_key, 0) + overlap
        if not candidates:
            continue
        best_key, best_overlap = max(candidates.items(), key=lambda item: (item[1], item[0]))
        if best_overlap <= 0:
            continue
        if best_key != segment.speaker_key:
            result[str(segment.id)] = best_key
    return result


class MeetingDiarizationOverlay:
    """会议分人 overlay：缓冲 PCM，会末提交非流式 diarize（legacy 诊断）。

    线程安全模型：所有状态都在 asyncio 单线程事件循环中访问（PCM 由
    ``push_pcm`` 同步回调、flush 为 async 任务），无需额外锁。
    """

    def __init__(
        self,
        *,
        transcriber: BatchTranscriber | None = None,
        group_id: str | None = None,
        max_buffer_seconds: int = 1800,
    ) -> None:
        self._transcriber = transcriber
        self._group_id = group_id or ""
        self._max_bytes = max(1, max_buffer_seconds) * _BYTES_PER_SEC
        self._chunks: deque[bytes] = deque()
        self._buffered_bytes = 0
        self._buffer_start_sample = 0
        self._total_samples = 0
        self._active = False

    @property
    def active(self) -> bool:
        return self._active

    @property
    def buffer_start_sample(self) -> int:
        """当前缓冲首字节对应的会议样本起点（未被裁剪时为 0）。"""
        return self._buffer_start_sample

    def start(self, *, group_id: str | None = None) -> None:
        self._group_id = group_id or self._group_id or ""
        self._chunks.clear()
        self._buffered_bytes = 0
        self._buffer_start_sample = 0
        self._total_samples = 0
        self._active = True

    def stop(self) -> None:
        self._active = False
        self._chunks.clear()
        self._buffered_bytes = 0

    def push_pcm(self, data: bytes) -> None:
        """同步接收一个 PCM chunk（从 SubtitleProxy 音频监听器调用）。"""
        if not self._active or not data:
            return
        self._chunks.append(data)
        self._buffered_bytes += len(data)
        self._total_samples += len(data) // 2
        # 丢弃最旧分块并推进缓冲起点，避免长会议内存无界增长；
        # 始终保留至少一个分块（单块超限时由 flush 前置校验可见失败）。
        while self._buffered_bytes > self._max_bytes and len(self._chunks) > 1:
            dropped = self._chunks.popleft()
            self._buffered_bytes -= len(dropped)
            self._buffer_start_sample += len(dropped) // 2

    def buffered_pcm(self) -> bytes:
        return b"".join(self._chunks)

    def reset_buffer(self) -> None:
        self._chunks.clear()
        self._buffered_bytes = 0
        self._buffer_start_sample = self._total_samples

    def clear(self) -> None:
        self.stop()

    async def flush(self) -> OverlayFlushResult:
        """把缓冲 PCM 提交到非流式 diarize，返回会议时间线说话人 span。

        内容不足最小阈值时返回空；请求超长在调用前拒绝并记录可见告警；
        分人失败吞掉异常返回空（不中断会议），但日志可见。
        """
        if not self._active or self._transcriber is None:
            return OverlayFlushResult()
        if self._buffered_bytes < _MIN_FLUSH_BYTES:
            return OverlayFlushResult()
        pcm, covered = self._drain()
        if len(pcm) > MAX_DIARIZE_PCM_BYTES:
            logger.warning(
                "MeetingDiarizationOverlay: 缓冲音频 %d 字节超过非流式请求长度上限"
                " %d，跳过本次诊断分人（失败可见，不静默声称完成）",
                len(pcm),
                MAX_DIARIZE_PCM_BYTES,
            )
            return OverlayFlushResult()
        try:
            result = await self._transcriber.transcribe_diarize(pcm)
        except Exception as exc:
            logger.warning("MeetingDiarizationOverlay: 非流式分人失败: %s", exc)
            return OverlayFlushResult()
        return _to_spans(result, self._group_id, covered)

    async def finish(self) -> OverlayFlushResult:
        """会末冲刷：清空缓冲并返回最后一段说话人 span（调用方负责整体归属）。"""
        if not self._active:
            return OverlayFlushResult()
        try:
            return await self.flush()
        finally:
            self._active = False
            self._chunks.clear()
            self._buffered_bytes = 0

    def _drain(self) -> tuple[bytes, tuple[int, int]]:
        """取出全部缓冲并返回 ``(pcm, 会议样本覆盖窗口)``。"""
        pcm = b"".join(self._chunks)
        covered = (
            self._buffer_start_sample,
            self._buffer_start_sample + self._buffered_bytes // 2,
        )
        self._chunks.clear()
        self._buffered_bytes = 0
        self._buffer_start_sample = self._total_samples
        return pcm, covered


def _to_spans(
    result: object,
    group_id: str,
    covered_samples: tuple[int, int],
) -> OverlayFlushResult:
    """把 batch transcriber 结果归一化为会议时间线说话人 span。

    batch 结果的时间是缓冲相对毫秒；这里统一加裁剪偏移换算到会议时间线，
    并丢弃落在覆盖窗口之外的 span（防御：跨裁剪边界不得产生归属）。
    """
    segments = getattr(result, "segments", None)
    covered_start_ms = covered_samples[0] // _SAMPLES_PER_MS
    covered_end_ms = covered_samples[1] // _SAMPLES_PER_MS
    if not segments:
        return OverlayFlushResult(
            covered_start_ms=covered_start_ms,
            covered_end_ms=covered_end_ms,
        )
    spans: list[SpeakerLabelledSpan] = []
    for seg in segments:
        raw_speaker = getattr(seg, "speaker", None)
        start_ms = int(getattr(seg, "start_ms", 0) or 0)
        end_ms = int(getattr(seg, "end_ms", 0) or 0)
        if not isinstance(raw_speaker, str) or not raw_speaker.strip():
            continue
        if end_ms <= start_ms:
            continue
        meeting_start, meeting_end = meeting_range(
            covered_samples[0],
            start_ms * _SAMPLES_PER_MS,
            end_ms * _SAMPLES_PER_MS,
        )
        span_start_ms = meeting_start // _SAMPLES_PER_MS
        span_end_ms = meeting_end // _SAMPLES_PER_MS
        if span_start_ms < covered_start_ms or span_end_ms > covered_end_ms:
            continue
        spans.append(
            SpeakerLabelledSpan(
                speaker_key=meeting_speaker_key(group_id, raw_speaker),
                raw_speaker=raw_speaker,
                start_ms=span_start_ms,
                end_ms=span_end_ms,
            )
        )
    return OverlayFlushResult(
        spans=tuple(spans),
        covered_start_ms=covered_start_ms,
        covered_end_ms=covered_end_ms,
    )


__all__ = [
    "BatchTranscriber",
    "MeetingDiarizationOverlay",
    "OverlayFlushResult",
    "SpeakerLabelledSpan",
    "assign_speakers_by_overlap",
    "meeting_diarization_group_id",
    "meeting_range",
    "meeting_speaker_key",
    "overlay_gate_for_runtime",
]
