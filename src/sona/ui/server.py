...
from sona.config import Settings, get_settings
from sona.logging import setup_logging
from sona.meeting.api import install_meeting_api
...
def create_app(
    ...
    *,
    ...
    app = FastAPI(title="Sona", version="1.5.0", lifespan=lifespan)
...
def _mount_static(app: FastAPI, static_dir: Path) -> None:
    ...
    if static_dir.is_dir() and (static_dir / "index.html").exists():
        ...
        async def placeholder() -> dict[str, str]:
            return {"message": "Sona 未构建（npm run build 于 ui/）"}
...
def main() -> None:
    ...
    setup_logging("ui")
    cfg = get_settings()
    logger.info("Sona 启动: http://%s:%s", cfg.ui.host, cfg.ui.port)
