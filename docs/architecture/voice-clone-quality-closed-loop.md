...
> **2026-09-12 实现边界更新：** 新工坊已分开参考/输出验收，并增加示例引导、显式生成参考注册、
> 手动输出检查和实际试听。录音链路不再执行客户端整段 RMS 归一；仅保留本地安全检查，原始字节
> 交由 SpeechRail 处理。下文中的高级降噪/声纹等属于后续设计，不表示已实现或实机通过。
> 当前客户端契约以[示例引导与双路径注册](voice-studio-guided-creation.md)为准。
