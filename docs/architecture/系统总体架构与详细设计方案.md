...
scope:
  ...
  - "sona.ui"
related_documents:
  - "docs/architecture/speaker-diarization-e2e-design.md"
  - "docs/operations/speechrail-openai-diarization-integration-acceptance.md"
...
> **系统定位**：全本地离线、超低延迟实时语音交互（Voice Assistant）+ 结构化会议助手（Meeting Assistant，含 SpeechRail 持续分人双通道、PostgreSQL ACID 持久化、异步 AI 纪要、崩溃恢复 Journal）+ 实时语音字幕（Live Subtitles）+ 会中内心 OS 伴侣。
...
> **更新时间**：2026-09-09 | **版本**：v2.5-SpeechRail-2.0.3

## 当前实现基线（2026-09-08）

- SpeechRail 是 ASR/TTS/Diarization 的唯一运行时：字幕与会议使用 `SpeechRailStreamingTranscriber`，语音助手使用 `SpeechRailConversationSTTProcessor` 与 `SpeechRailTTSService`；应用侧不加载或启动本地 ASR/TTS worker。
- 会议模式采用 **SPK-E2E-1 持续分人双通道与三大公理**：
  1. **正文不可变**：转录单元一旦确认（`CompletedItem`），文本内容与时间戳绝对不可变更，由 `append_completed_item` 写入 PostgreSQL；
  2. **分人原位修订**：SpeechRail v2 的 `speechrail.diarization.updated` 经 typed decoder 通过专用事务（`apply_speaker_patches`）按不可变 `segment_uid` 原位更新，推进分人水位；
  3. **人工更正绝对优先**：用户手动更正（`manually_corrected = true`）具有最高法律效力，禁止被后续任何自动分人 patch、平滑算法或 EOF 冲刷覆盖。
- **EOF 水位屏障隔离**：会议结束通过 Realtime 发送 `input_audio_buffer.commit` 作为 EOF，分人开启时屏障等待 `speechrail.diarization.done` 事件与仓储水位对齐（`RepositoryDiarizationGate.wait_persisted`）；未开启或不可用时直接按标准 ASR 终态封存。
- `/v1/realtime` 会话不可透明恢复。`SubtitleProxy` 重连或会议重新建连时创建新会话并递增 source epoch，再由窗口对账避免重复与丢字；不能把重连描述为服务端续传。
- **VAD 所有权按模式隔离**：语音助手在 Sona/Pipecat 的 `LLMUserAggregator` 内运行本地 Silero VAD（默认开始 `0.2s`、结束静音 `0.45s`），SpeechRail 该路径使用 `manual`；标准字幕由 SpeechRail `server_vad` 负责（请求静音 `400ms`），会议由 SpeechRail `server_vad` 负责（请求静音 `900ms`）。两种服务端窗口都使用 threshold `0.65`、prefix padding `300ms`。
- 服务端 VAD 的 `400/900ms` 是 endpointing 参数，不是 speaker diarization 的时间窗；SpeechRail 分人 activity 独立处理连续 PCM，完成后只修订 speaker 元数据。16kHz/512-sample 帧量化后，实际静音边界约为字幕 `416ms`、会议 `928ms`。
- 当前联调服务为 SpeechRail `2.0.3` quality profile：`auto → silero`，`speech_admission_enabled=true`，CoreML Sortformer FP16 diarization ready。SpeechRail 修改必须从其源码仓库构建 wheel 后通过 managed installer 发布。
...
flowchart TB
    ...
    subgraph L3[3. 算法与模型服务层 Algorithms & Model Services]
        VAD_M[SONA 交互模式本地 Silero VAD<br/>字幕/会议不重复运行]
    ...
    INJECT --> PIPE
    PROXY --> ASR_M
    ASR_M -. server_vad=400/900<br/>按 subtitle/meeting 选择 .-> ENDPOINT[SpeechRail endpointing]
    ASR_M --> DIAR_M
...
3. **`completed`**：标准 ASR `completed` 与（若启用分人）`speechrail.diarization.done` 及仓储水位收尾完成，会议标记为已完成，并自动向 `meeting_minutes` 插入 `queued` 状态的 AI 纪要任务；
...
│ FINALIZING      │ 向 SpeechRail 发送 `input_audio_buffer.commit`；分人时等待 `done` 与水位；   │
...
flowchart LR
    ...
    subgraph SPEECHRAIL["SpeechRail OpenAI Realtime /v1 (:8201)"]
        SR_INPUT --> QWEN_ASR["SpeechRail ASR profile<br/>OpenAI input_audio_transcription.*"]
...
UI 主进程内的 [`SubtitleProxy`](../../src/sona/subtitles/proxy.py) 是字幕服务与上层业务的唯一网关：
...
sequenceDiagram
    ...
    SR-->>RT: speechrail.diarization.done（分人尾部冲刷与水位终态）
...
[`MeetingSummaryService`](../../src/sona/meeting/summary/service.py) 在会议结束后调用 `local/kat-coder-2.5` 模型生成结构化纪要。为了确保纪要内容的严谨性与防幻觉，输出必须遵循严格的 JSON Schema 契约（`contracts/meeting-assistant/v1/schemas/minutes-content.schema.json`），并为每个事实点锚定转录片段的 UUID。
...
| `GET /v1/voices` | HTTP REST | 代理 TTS 音色列表 | 返回系统预置与自建音色 profiles |
| `POST /v1/voices` | HTTP REST | 代理创建自定义音色 | 自然语言设计并持久化新音色 |
| `DELETE /v1/voices/{id}` | HTTP REST | 代理删除自定义音色 | 安全删除自建音色（系统音色受保护不可删） |
...
## 声音工坊双路径注册与职责边界（2026-09-12 增量）

Sona 的描述式音色创建由示例模板、可编辑表单和有界 JSON 代理组成。Quality 档上，
SpeechRail 执行 VoiceDesign 参考生成、规范化、ASR 核验及 Base-backed 注册；Sona 不加载模型。
录音式创建提交原始录音和实际参考文本，前端不重复施加整段 RMS 增益。

创建成功仅意味着参考已注册，不能改变当前播报音色或补造输出质量。结果页持有独立 reference
与 synthesis 状态，依次由用户触发 18 段检查、实际试听及手动确认。正式激活仍经已有 `set_voice`
控制命令与服务端 ack，不绕过运行时状态源。未知注册结果保留同一 ID/正文并先核对已保存资产。

新增的 `/v1/voices/designs` 代理复用模式守卫、配置认证及稳定错误信封，输入总量限制 64 KiB；
旧 instruction API 和旧资产保持兼容。详细结构、示例、状态机、失败恢复及验收范围见
[声音工坊：示例引导与双路径注册](voice-studio-guided-creation.md)。
