---
title: "Sona UI 设计方案"
description: "Sona 前端控制台架构设计、单源麦克风控制面、组件状态机、WebSocket 协议桥接与交互设计规范"
status: active
type: guide
category: frontend
version: "v1.1.0"
...
Sona 是本机语音系统的默认运行入口，不只是展示页。`sona-ui` 同时拥有：

- 唯一麦克风采集器 `AudioHub`；
- 注入式 Pipecat 交互会话；
- SpeechRail OpenAI Realtime `/v1/realtime` 字幕/会议 ASR 代理；
- 浏览器事件面、控制面和静态资源。

浏览器不采集音频，只展示服务端事件并发送严格控制命令。关闭浏览器不会停止服务。
`sona-interact` 仅用于无 UI 模式，与 `sona-ui` 通过文件锁互斥。
...
flowchart LR
    ...
    SP --> SR[SpeechRail OpenAI Realtime /v1 :8201<br/>ASR / diarization profile]
...
历史上“同时运行 sona-ui 与 sona-interact 的多服务冒烟”结论已废止；当前拓扑由 `sona-ui`、SpeechRail、LM Studio 与 PostgreSQL 构成，
headless 入口只能替代 UI 入口。


## 声音工坊新注册流程（2026-09-12 增量）

主声音工坊维持“克隆声音 / 描述声音”两个入口，描述入口改为六类示例卡片 → 可编辑描述 →
参考文本 → 可选草稿试听 → 显式生成注册。选中状态用文字与边框标示；已有修改必须确认替换，
切换两个标签页保留当前草稿，seed 放入高级设置。窄窗口使用单列及模态内滚动，明暗主题继承现有 token。

两类新音色都在保存后进入统一结果页，参考和合成输出分别显示。18 段输出检查由用户触发，
只有完整输出证据及实际试听完成后才在工坊中允许确认使用。保存不自动激活，失败不会丢弃已保存音色。
生成参考的显示类别是“设计”，推理 mode 仍是 `clone`，不能用显示类别重写 Base 路由。

录音保留原始字节，提词文本固定后可人工纠正为实际朗读；不再在浏览器侧按整段 RMS 增益。
请求或播放进行时锁定切换、删除、关闭；组件销毁取消等待/播放并释放资源。模态焦点保留在内部，
退出恢复调用方焦点。保留原助手静音租约与会议/字幕互斥。

完整状态和代理契约见[声音工坊：示例引导与双路径注册](../architecture/voice-studio-guided-creation.md)。
新服务器依赖及未完成的 Safari/WebKit、真实设备和听感验证均以该文档为准。
